...
class ClaudeAnalyzerV2:
    ...
    def _get_demo_analysis(self, matches: List[Dict[str, Any]]) -> str:
        ...
        for i, match in enumerate(matches[:10], 1):  # Максимум 10 матчей
            ...
            # Определяем рекомендацию на основе коэффициентов
            if min(p1, p2) < 1.3:
                recommendation = "ПРОПУСК"
                reason = "Слишком низкие коэффициенты, нет value"
                risk = "низкий 🟢"
            elif abs(p1 - p2) < 0.3:
                recommendation = "СТАВКА на X (ничья)"
                reason = "Команды равной силы, ничья недооценена"
                risk = "средний 🟡"
            elif min(p1, p2) > 3.0:
                recommendation = "ПРОПУСК"
                ...
                recommendation = f"СТАВКА на {'П1' if p1 < p2 else 'П2'}"
                reason = "Умеренный фаворит с хорошим value"
                risk = "средний 🟡"
