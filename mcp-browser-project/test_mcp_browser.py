"""
Тест MCP Browser Python SDK для получения данных с BetBoom
"""
import asyncio
import logging
...
logger = logging.getLogger('mcp_browser_test')

class MCPBrowserClient:
    """MCP клиент для работы с browser-use"""
    ...
    async def connect(self):
        ...
        try:
            self.logger.info("🔌 Подключение к MCP серверу...")
...
class MCPBetBoomScraper:
    """Скрапер BetBoom через MCP Browser"""
...
async def test_mcp_browser_betboom():
    """Тестирование MCP Browser для BetBoom"""
    logger.info("🚀 Тестирование MCP Browser для BetBoom")
    ...
    try:
        ...
        logger.info("\n📸 Создание скриншота...")
        if await scraper.mcp_client.take_screenshot("betboom_screenshot.png"):
            logger.info("✅ Скриншот создан: betboom_screenshot.png")
        else:
...
if __name__ == "__main__":
    asyncio.run(test_mcp_browser_betboom())